...
precedent (board-level recompute triggered from `apply_piece_post_effects`).
...
Then in `apply_piece_post_effects` (phase 2 of the make_move pipeline), scan
for pressure plates the moved piece settled onto:
...
4. PressurePlate + the post-move scan in `apply_piece_post_effects`.
5. Tests for all of the above. Plan 09 picks up the Junction state for
   train traversal.

## Implementation notes (post-landing)

All five build-order steps landed:

- **Square types** — `SquareType::{Switch, Junction, Gate, PressurePlate}`
  with their payloads (`targets`, `state` + `branches`, `open`, `fires_for`),
  plus the `is_walkable` arms. FEN round-trips through the `T=...` /
  `ID=...` / `STATE=...` / `BRANCHES=...` / `TARGETS=...` / `OPEN=...` /
  `FIRES=...` keys (see the README FEN appendix for the full list).
- **`ThrowSwitch` move type** — generated by `Board::get_moves` for any
  piece sitting on a Switch tile (gated on the piece's
  `can_throw_switch()` trait method, default `true`).
- **`signal.rs`** — `fire_signal` + `activate_receiver`. Bounded
  propagation (receivers never themselves emit during the call), so
  cascades are bounded by the call stack from the originating switch or
  plate. Same-id Junction+Gate coexist (both fire).
- **PressurePlate** — `maybe_fire_pressure_plate` runs from
  `apply_piece_post_effects` on every landing of the moving piece(s).
  Round-3 wired it into `advance_trains` too so a `FIRES=N` (Neutral)
  plate becomes reachable the moment a future tile type lets a cart
  settle on a plate.

The v1-deferred items (diagonal track / Switch routing, dual-state
switches with `ThrowSwitchToState`, color-restricted switches, editor
wiring visualisation) remain deferred.
